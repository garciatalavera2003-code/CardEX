...
async function load(s,file){
  ...
  try{
    ...
    s.worker=new Worker('detector.js?v=0.3.1');
...
let probe;
function checkEngine(){probe?.terminate();$('retry').hidden=true;$('engine').textContent='Preparando detector…';probe=new Worker('detector.js?v=0.3.1');const timer=setTimeout(()=>finish(false),45000);function finish(ok){clearTimeout(timer);probe.terminate();$('engine').textContent=ok?'Detector disponible · 0.3.1':'Detector no disponible';$('retry').hidden=ok;}probe.onmessage=e=>finish(!e.data.error);probe.onerror=()=>finish(false);probe.postMessage({probe:true});}
