...
for (const [key, name, number] of [['front', 'Anverso', 'A'], ['back', 'Reverso', 'B']]) {
  ...
  section.innerHTML = `<div class="side-head"><h2><span>${number}</span>${name}</h2><span class="badge">Sin fotografía</span></div><div class="drop"><div class="empty"><span class="upload-icon" aria-hidden="true">＋</span><div>La carta, por ${key === 'front' ? 'delante' : 'detrás'}</div><p>Arrastra aquí tu fotografía</p></div><canvas hidden aria-label="Fotografía del ${name.toLowerCase()} y contorno, si se detecta"></canvas><button type="button" class="camera-open" data-camera="${key}">Usar cámara</button><label class="choose">Elegir ${name.toLowerCase()}<input aria-label="Elegir fotografía del ${name.toLowerCase()}" type="file" accept="image/jpeg,image/png,image/webp"></label><p class="format">JPG, PNG o WebP · Hasta 20 MB</p></div><div class="actions"><span class="filename">Una foto de una sola carta</span><button type="button" class="remove-photo" hidden>Quitar</button></div><div class="side-result" role="status"><strong>Pendiente de fotografía</strong><p>Se comprobarán el contorno y la calidad visible.</p></div>`;
  ...
  section.querySelector('.remove-photo').addEventListener('click', () => reset(s));
...
function reset(s){stop(s);s.result=null;s.hash=null;s.state='empty';s.canvas.hidden=true;s.canvas.width=1;s.canvas.height=1;s.drop.classList.remove('has-image');s.section.querySelector('.remove-photo').hidden=true;s.section.querySelector('.filename').textContent='Una foto de una sola carta';show(s,'empty','Pendiente de fotografía','Se comprobarán el contorno y la calidad visible.','Sin fotografía');}
...
async function load(s,file){
  ...
  try{
    ...
    s.canvas.hidden=false;s.drop.classList.add('has-image');s.section.querySelector('.remove-photo').hidden=false;s.section.querySelector('.filename').textContent=`${file.name} · ${bitmap.width} × ${bitmap.height} px`;
    ...
    s.worker=new Worker('detector.js?v=0.4.0');
    ...
    s.worker.onmessage=({data})=>{if(seq!==s.seq)return;if(data.error){fail(data.error);return;}clearTimeout(s.timer);s.worker.terminate();s.worker=null;s.result=data;
      const path=data.outline||data.points;
      if(path){ctx.strokeStyle=data.state==='compatible'?'#dafa76':'#efc47f';ctx.lineWidth=Math.max(2,s.canvas.width/380);ctx.setLineDash(data.outline?[]:[8,6]);ctx.beginPath();path.forEach((p,i)=>i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y));ctx.closePath();ctx.stroke();ctx.setLineDash([]);}
      show(s,data.state,data.title,data.message,data.state==='compatible'?'Contorno compatible':(data.points?'Revisar contorno':'Sin confirmar'));
    };
...
let probe;
function checkEngine(){probe?.terminate();$('retry').hidden=true;$('engine').textContent='Preparando detector…';probe=new Worker('detector.js?v=0.4.0');const timer=setTimeout(()=>finish(false),45000);function finish(ok){clearTimeout(timer);probe.terminate();$('engine').textContent=ok?'Detector disponible · 0.4.0':'Detector no disponible';$('retry').hidden=ok;}probe.onmessage=e=>finish(!e.data.error);probe.onerror=()=>finish(false);probe.postMessage({probe:true});}
